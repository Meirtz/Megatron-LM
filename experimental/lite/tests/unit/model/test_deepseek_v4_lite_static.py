...
def test_deepseek_v4_checkpoint_keeps_mhc_head_scalars_replicated():
    pytest.importorskip("torch")

    from megatron.lite.model.deepseek_v4.lite.checkpoint import PLACEMENT_FN

    for name in ("hc_head.hc_scale", "hc_head.hc_base", "mtp.0.hc_head.hc_scale"):
        placements = PLACEMENT_FN(name)
        assert [type(item).__name__ for item in placements] == [
            "Replicate",
            "Replicate",
            "Replicate",
            "Replicate",
