...
"""PEFT adapter import/export for GLM5 lite native LoRA."""
...
from megatron.lite.model.glm5.lite.adapter_semantics import (
    ADAPTER_META_FORMAT as _ADAPTER_META_FORMAT,
    ADAPTER_TARGET_MODULES as _ADAPTER_TARGET_MODULES,
    LORA_METADATA_FIELDS as _LORA_METADATA_FIELDS,
    MODEL_METADATA_FIELDS as _MODEL_METADATA_FIELDS,
    PARALLEL_METADATA_FIELDS as _PARALLEL_METADATA_FIELDS,
    PEFT_PREFIX as _PEFT_PREFIX,
    TARGET_MODULE_EXPANSIONS as _TARGET_MODULE_EXPANSIONS,
)
...
def _unwrap_model(module: nn.Module) -> nn.Module:
    ...
    for _ in range(8):
        ...
        if (
            isinstance(inner_model, nn.Module)
            and isinstance(getattr(current, "config", None), Glm5Config)
            and hasattr(inner_model, "layer_indices")
        ):
            return inner_model
        next_model = None
...
def _iter_glm5_chunks(chunks: list[nn.Module] | tuple[nn.Module, ...]) -> list[nn.Module]:
    return [_unwrap_model(chunk) for chunk in chunks]


def _iter_transformer_layers(chunks: list[nn.Module] | tuple[nn.Module, ...]):
    for chunk in _iter_glm5_chunks(list(chunks)):
        for layer in chunk.layers:
            yield layer
        mtp = getattr(chunk, "mtp", None)
        if mtp is not None:
            for mtp_layer in mtp.layers:
                yield mtp_layer.transformer_layer


def _iter_adapter_layers(
    chunks: list[nn.Module] | tuple[nn.Module, ...], model_cfg: Glm5Config
):
    for chunk in _iter_glm5_chunks(list(chunks)):
        for layer in chunk.layers:
            yield int(layer.layer_idx), layer
        mtp = getattr(chunk, "mtp", None)
        if mtp is not None:
            for mtp_idx, mtp_layer in enumerate(mtp.layers):
                yield model_cfg.num_hidden_layers + mtp_idx, mtp_layer.transformer_layer


def _expert_lora_representation(
    chunks: list[nn.Module] | tuple[nn.Module, ...], model_cfg: Glm5Config
) -> str:
    has_shared = any(
        _expert_lora_is_shared(getattr(layer.moe.experts, attr))
        for _, layer in _iter_adapter_layers(chunks, model_cfg)
...
def _dense_mlp_key(layer_idx: int, module: str, suffix: str) -> str:
...
def _shared_expert_key(layer_idx: int, module: str, suffix: str) -> str:
    return f"{_layer_prefix(layer_idx)}.mlp.shared_experts.{module}.{suffix}.weight"
...
def _is_supported_adapter_tensor_key(key: str) -> bool:
    ...
    if len(parts) == 4 and parts[1] == "mlp" and parts[2] == "shared_experts":
        return parts[3] in mlp_targets
    return False
...
def _iter_native_lora_modules(chunks: list[nn.Module] | tuple[nn.Module, ...]):
    for layer in _iter_transformer_layers(chunks):
        dsa = layer.self_attention.self_attention
        for attr in ("q_a_lora", "q_b_lora", "kv_a_lora", "kv_b_lora", "o_lora"):
            module = getattr(dsa, attr, None)
        ...
        if layer.moe is not None:
            ...
            for attr in ("fc1_lora", "fc2_lora"):
                module = getattr(experts, attr, None)
                if module is not None:
                    yield module
            shared = layer.moe.shared_expert
            for attr in ("gate_up_lora", "down_lora"):
                module = getattr(shared, attr, None)
                if module is not None:
                    yield module
...
def _validate_adapter_only_config(adapter_config: dict[str, Any]) -> None:
    ...
    if modules_to_save not in (None, [], ()):
        raise ValueError(
            "Adapter config modules_to_save is not supported for adapter-only GLM5 LoRA."
        )
...
def _validate_adapter_config(
    ...
    *,
    ...
    if config_alpha is not None and native_alpha is not None and not _numbers_match(
        config_alpha, native_alpha
    ):
        raise ValueError(
...
def _validate_expected_lora_config(
    ...
    native_alpha = _infer_native_alpha(chunks)
    if native_alpha is not None and not _numbers_match(native_alpha, _effective_lora_alpha(expected)):
        raise ValueError(
...
def _validate_adapter_metadata(
    ...
    if metadata_format != _ADAPTER_META_FORMAT:
        raise ValueError(
            f"Expected Megatron Lite GLM5 adapter metadata format {_ADAPTER_META_FORMAT!r}, "
    ...
    if metadata_representation is not None:
        expected_representation = _expert_lora_representation(chunks, model_cfg)
    ...
    validate_lora_rank_value(parallel.get("ep"), key="Adapter metadata parallel.ep")
    for key in ("tp", "pp", "etp"):
        value = validate_lora_rank_value(parallel.get(key), key=f"Adapter metadata parallel.{key}")
        if value != 1:
            raise ValueError(
                f"Adapter metadata parallel.{key}={value} is not supported; expected 1."
            )
...
def _validate_parallel_scope(ps: ParallelState) -> None:
    if ps.tp_size != 1:
        raise NotImplementedError("GLM5 LoRA adapter import/export currently supports tp=1.")
    if ps.pp_size != 1:
        raise NotImplementedError("GLM5 LoRA adapter import/export currently supports pp=1.")
    if ps.etp_size != 1:
        raise NotImplementedError("GLM5 LoRA adapter import/export currently supports etp=1.")
...
def _materialize_export_tensor(
    ...
    *,
    ...
    FSDP2 may expose parameters as DTensor-like values.  Exporting ``to_local()``
    would persist only this rank's shard, so prefer ``full_tensor()`` whenever
    the object provides it.
    """
    ...
    if expected_ndim is not None and tensor.ndim != expected_ndim:
        raise ValueError(
            f"LoRA adapter export tensor {name!r} must be {expected_ndim}-D after materialization, "
            f"got shape {tuple(tensor.shape)}."
        )
    if cpu:
        tensor = tensor.cpu()
...
def initialize_lora_olora_tail(
    ...
    *,
    ...
    """Initialize supported GLM5 LoRA modules from base-weight tail singular vectors.

    This is a post-base-load pass for the paper's OLoRA-tail initializer:
    ``B0 = U_tail`` and ``A0 = V_tail.T`` from the smallest singular-vector
    subspace of each selected base weight. Singular values are intentionally
    not applied.
    """
    ...
    for layer_idx, layer in _iter_adapter_layers(chunks, model_cfg):
        dsa = layer.self_attention.self_attention
        for name, base_module, lora in (
            ("q_a_proj", dsa.q_a_proj, dsa.q_a_lora),
            ("q_b_proj", dsa.q_b_proj, dsa.q_b_lora),
            ("kv_a_proj_with_mqa", dsa.kv_a_proj_with_mqa, dsa.kv_a_lora),
            ("kv_b_proj", dsa.kv_b_proj, dsa.kv_b_lora),
            ("o_proj", dsa.o_proj, dsa.o_lora),
        ):
            full_name = f"layers.{layer_idx}.self_attn.{name}"
            if _init_linear_lora_olora_tail(
        ...
        if layer.mlp is not None:
            for name, base_module, lora in (
                ("gate_up_proj", layer.mlp.gate_up, layer.mlp.gate_up_lora),
                ("down_proj", layer.mlp.down, layer.mlp.down_lora),
            ):
        ...
        if layer.moe is not None:
            ...
            shared = layer.moe.shared_expert
            for name, base_module, lora in (
                ("gate_up_proj", shared.gate_up, shared.gate_up_lora),
                ("down_proj", shared.down, shared.down_lora),
            ):
                full_name = f"layers.{layer_idx}.mlp.shared_experts.{name}"
                if _init_linear_lora_olora_tail(
                    lora, _base_weight(base_module), full_name, force=force
                ):
                    initialized.append(full_name)
...
def _export_linear_lora(
    ...
    if lora is None:
        return
    key_a = key_fn(module_name, "lora_A")
    key_b = key_fn(module_name, "lora_B")
    state[key_a] = _materialize_export_tensor(lora.lora_a, name=key_a)
    state[key_b] = _materialize_export_tensor(lora.lora_b, name=key_b)


def _export_gate_up_lora(state: dict[str, torch.Tensor], key_fn, lora) -> None:
    if lora is None:
        return
    gate_a_key = key_fn("gate_proj", "lora_A")
    gate_b_key = key_fn("gate_proj", "lora_B")
    up_a_key = key_fn("up_proj", "lora_A")
    up_b_key = key_fn("up_proj", "lora_B")
    lora_a = _materialize_export_tensor(lora.lora_a, name=gate_a_key)
    lora_b = _materialize_export_tensor(lora.lora_b, name=f"{gate_b_key}/{up_b_key}")
    gate_b, up_b = lora_b.chunk(2, dim=0)
    state[gate_a_key] = lora_a
    state[gate_b_key] = gate_b.contiguous()
    state[up_a_key] = lora_a.clone()
    state[up_b_key] = up_b.contiguous()
...
def export_lora_adapter_state(
    ...
    *,
    ...
    """Export native GLM5 LoRA tensors to a PEFT-style adapter state dict."""
    ...
    for layer_idx, layer in _iter_adapter_layers(chunks, model_cfg):
        ...
        _export_linear_lora(
            state,
            lambda module, suffix, idx=layer_idx: _attn_key(idx, module, suffix),
            "q_a_proj",
            dsa.q_a_lora,
        )
        _export_linear_lora(
        ...
        if layer.mlp is not None:
            _export_gate_up_lora(
                state,
                lambda module, suffix, idx=layer_idx: _dense_mlp_key(idx, module, suffix),
        ...
        if layer.moe is not None:
            ...
            _export_gate_up_lora(
                state,
                lambda module, suffix, idx=layer_idx: _shared_expert_key(idx, module, suffix),
                shared.gate_up_lora,
            )
            _export_linear_lora(
                state,
...
def save_lora_adapter(
    ...
    *,
    ...
    """Save a PEFT/Mint-compatible GLM5 LoRA adapter directory."""
    ...
    if _rank() == 0:
        ...
        meta = {
            ...
            "base_model_name_or_path": base_model_name_or_path,
            "expert_lora_representation": _expert_lora_representation(chunks, model_cfg),
            ...
            "model": {
                ...
                "num_nextn_predict_layers": model_cfg.num_nextn_predict_layers,
                "mtp_use_repeated_layer": model_cfg.mtp_use_repeated_layer,
            },
            "metadata": {
                **adapter_metadata,
                "init_lora_weights": init_lora_weights_value,
        ...
        export_lora_adapter_state(chunks, model_cfg, ps, cpu=True)
        result = {}
...
def _to_lora_param(
    ...
    return _require_tensor_shape(state, key, tuple(param.shape), consumed_keys).to(
...
def _load_linear_lora(
    ...
    _copy_to_lora_param(
        lora.lora_a,
        _to_lora_param(state, key_a, lora.lora_a, consumed_keys),
        key=key_a,
    )
    _copy_to_lora_param(
        lora.lora_b,
        _to_lora_param(state, key_b, lora.lora_b, consumed_keys),
        key=key_b,
    )
    return 2


def _load_gate_up_lora(
    state: dict[str, torch.Tensor],
    key_fn,
    lora,
    *,
    ...
    if lora is None:
        return 0
    gate_a = _to_lora_param(state, key_fn("gate_proj", "lora_A"), lora.lora_a, consumed_keys)
    up_a = _to_lora_param(state, key_fn("up_proj", "lora_A"), lora.lora_a, consumed_keys)
    if not torch.equal(gate_a, up_a):
        raise ValueError(
            "Fused gate/up LoRA requires gate_proj and up_proj lora_A tensors to match."
        )
    if lora.lora_b.shape[0] % 2 != 0:
        raise ValueError(
            f"Fused gate/up LoRA B tensor must have an even output dimension, got {tuple(lora.lora_b.shape)}."
        )
    b_shape = (lora.lora_b.shape[0] // 2, lora.lora_b.shape[1])
    gate_b = _require_tensor_shape(
        state, key_fn("gate_proj", "lora_B"), b_shape, consumed_keys
    ).to(device=lora.lora_b.device, dtype=lora.lora_b.dtype)
    up_b = _require_tensor_shape(
        state, key_fn("up_proj", "lora_B"), b_shape, consumed_keys
    ).to(
        device=lora.lora_b.device,
        dtype=lora.lora_b.dtype,
    )
    _copy_to_lora_param(lora.lora_a, gate_a, key=key_fn("gate_proj", "lora_A"))
    _copy_to_lora_param(
        lora.lora_b,
        torch.cat([gate_b, up_b], dim=0),
        key=f"{key_fn('gate_proj', 'lora_B')}/{key_fn('up_proj', 'lora_B')}",
    )
    return 4


def _validate_all_expert_adapter_keys(
    state: dict[str, torch.Tensor],
    ...
    if experts.fc1_lora is not None:
        if _expert_lora_is_shared(experts.fc1_lora):
            if experts.fc1_lora.lora_b.shape[0] % 2 != 0:
                raise ValueError(
                    "GLM5 fused shared expert fc1_lora B tensor must have an even output "
                    f"dimension, got {tuple(experts.fc1_lora.lora_b.shape)}."
                )
            ...
            if experts.fc1_lora.lora_b.shape[1] % 2 != 0:
                raise ValueError(
                    "GLM5 fused expert fc1_lora B tensor must have an even output "
                    f"dimension, got {tuple(experts.fc1_lora.lora_b.shape)}."
                )
...
def load_lora_adapter_state(
    ...
    *,
    ...
    """Load a PEFT adapter state into this rank's native GLM5 LoRA tensors."""
        ...
        if layer.moe is not None:
            ...
            if experts.fc1_lora is not None:
                if _expert_lora_is_shared(experts.fc1_lora):
                    ...
                    local_gate_b = []
                    local_up_b = []
                    for expert_idx in range(expert_start, expert_start + experts.num_local_experts):
                        ...
                        if not torch.equal(gate_a, up_a):
                            raise ValueError(
                                "GLM5 fused expert fc1_lora requires gate/up lora_A "
                                "tensors to match."
                            )
                    ...
                    if any(not torch.equal(local_gate_a[0], value) for value in local_gate_a[1:]):
                        raise ValueError(
                            "GLM5 shared expert fc1_lora can only import PEFT adapters "
                    ...
                    loaded += 4 * experts.num_local_experts
                else:
                    if experts.fc1_lora.lora_b.shape[1] % 2 != 0:
                        raise ValueError(
                            "GLM5 fused expert fc1_lora B tensor must have an even output "
                    ...
                    _copy_to_lora_param(
                        ...
                        key=(
                            ...
                            f"{_expert_key(layer_idx, expert_start + experts.num_local_experts - 1, 'gate_proj', 'lora_A')}"
                        ),
                    )
                    _copy_to_lora_param(
                        experts.fc1_lora.lora_b,
                        ...
                        key=(
                            f"{_expert_key(layer_idx, expert_start, 'gate_proj', 'lora_B')}/"
                            f"{_expert_key(layer_idx, expert_start, 'up_proj', 'lora_B')}.."
                            f"{_expert_key(layer_idx, expert_start + experts.num_local_experts - 1, 'gate_proj', 'lora_B')}/"
                            f"{_expert_key(layer_idx, expert_start + experts.num_local_experts - 1, 'up_proj', 'lora_B')}"
                        ),
                    )
                    loaded += 4 * experts.num_local_experts
            if experts.fc2_lora is not None:
                if _expert_lora_is_shared(experts.fc2_lora):
                    ...
                    for expert_idx in range(expert_start, expert_start + experts.num_local_experts):
                        local_a.append(
                            _require_tensor_shape(
                                state,
                                _expert_key(layer_idx, expert_idx, "down_proj", "lora_A"),
                                tuple(experts.fc2_lora.lora_a.shape),
                                consumed_keys,
                            ).to(
                        ...
                        local_b.append(
                            _require_tensor_shape(
                                state,
                                _expert_key(layer_idx, expert_idx, "down_proj", "lora_B"),
                                tuple(experts.fc2_lora.lora_b.shape),
                                consumed_keys,
                            ).to(
                    ...
                    if any(not torch.equal(local_a[0], value) for value in local_a[1:]):
                        raise ValueError(
                            "GLM5 shared expert fc2_lora can only import PEFT adapters "
                    ...
                    for expert_idx in range(expert_start, expert_start + experts.num_local_experts):
                        local_fc2_a.append(
                            _require_tensor_shape(
                                state,
                                _expert_key(layer_idx, expert_idx, "down_proj", "lora_A"),
                                tuple(experts.fc2_lora.lora_a.shape[1:]),
                                consumed_keys,
                            ).to(
                                device=experts.fc2_lora.lora_a.device,
                                dtype=experts.fc2_lora.lora_a.dtype,
                        ...
                        local_fc2_b.append(
                            _require_tensor_shape(
                                state,
                                _expert_key(layer_idx, expert_idx, "down_proj", "lora_B"),
                                tuple(experts.fc2_lora.lora_b.shape[1:]),
                                consumed_keys,
                            ).to(
                                device=experts.fc2_lora.lora_b.device,
                                dtype=experts.fc2_lora.lora_b.dtype,
                    ...
                    _copy_to_lora_param(
                        ...
                        key=(
                            ...
                            f"{_expert_key(layer_idx, expert_start + experts.num_local_experts - 1, 'down_proj', 'lora_A')}"
                        ),
                    )
                    _copy_to_lora_param(
                        experts.fc2_lora.lora_b,
                        ...
                        key=(
                            ...
                            f"{_expert_key(layer_idx, expert_start + experts.num_local_experts - 1, 'down_proj', 'lora_B')}"
                        ),
                    )
                    loaded += 2 * experts.num_local_experts

            if strict:
                _validate_all_expert_adapter_keys(
                    state,
                    consumed_keys,
                    layer_idx=layer_idx,
                    model_cfg=model_cfg,
